...
class BankAdministration:
    ...
    def get_accounts_of_customer(self, customer):
        ...
        accounts = self.__account_mapper.find_by_owner_id(customer.get_id())
        for a in accounts:
            source_transactions = self.__transaction_mapper.find_by_source_account_id(a.get_id())
            target_transactions = self.__transaction_mapper.find_by_target_account_id(a.get_id())
            transactions = source_transactions + target_transactions
            a.set_transactions(transactions)
    ...
    def create_transaction_for(self, amount, source_account, target_account, text):
        """
        Erstellen einer neuen Buchung.
        :param amount: der Geldwert dieser Buchung
        :param source_account: das Quellkonto
        :param target_account: das Zielkonto
        :param text: Der Buchungstext
        :return: die Transaktion
        """

        # Ihr Code für Aufgabe 5
        t = Transaction()
        t.set_amount(amount)
        ...
        self.__transaction_mapper.insert(t)
        return t
